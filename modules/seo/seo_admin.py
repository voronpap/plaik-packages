"""SEO Admin JSON commands. Mapping in, Mapping out."""
...
def register_admin(runtime: ExtensionRuntime, engine: Any) -> None:
    runtime.admin.register(
        "seo.records.list",
        _wrap(lambda _payload: engine.list_records()),
    )
    runtime.admin.register(
        "seo.records.get",
        _wrap(lambda payload: engine.get_record(_require_id(payload))),
    )
    runtime.admin.register(
        "seo.records.set",
        _wrap(
            lambda payload: engine.set_record(
                _require_id(payload),
                payload.get("title"),
