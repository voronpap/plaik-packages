"""Inventory Admin JSON commands. Mapping in, Mapping out."""
...
def register_admin(runtime: ExtensionRuntime, engine: Any) -> None:
    runtime.admin.register(
        ...
        _wrap(lambda _payload: engine.list_stock()),
    )
    runtime.admin.register(
        "inventory.stock.get",
        _wrap(lambda payload: engine.get_stock(_require_id(payload))),
    )
    runtime.admin.register(
        "inventory.stock.set",
        _wrap(
            lambda payload: engine.set_stock(
                _require_id(payload),
    ...
    runtime.admin.register(
        ...
        _wrap(
            lambda payload: engine.adjust_stock(
                _require_id(payload),
                payload.get("delta"),
            )
        ),
