...
Official PLAIK cart module 1.0.0. Provides `cart.lines`. Cart identity is `(store_id, cart_id)` where `cart_id` is a `ResourceRef.id` string. Close-gate carts are Admin-managed; there is no guest session owner and no `owner_id` payload.
