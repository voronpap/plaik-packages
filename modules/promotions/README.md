...
Official PLAIK promotions module 1.0.0. Provides `promotions.coupons`. Coupon identity is `(store_id, coupon_id)` with a unique `code` per store. Apply returns a cart-level discount against a quote and does not write `pricing.list`.
